...
imageToDispL = './depth_tune_240p/left_0.png'
imageToDispR = './depth_tune_240p/right_0.png'
...
mode_sobel = False
if mode_sobel:
    sobelX = cv2.Sobel(pair_img_L, cv2.CV_16S, 1, 0, ksize=3)
    sobelY = cv2.Sobel(pair_img_L, cv2.CV_16S, 0, 1, ksize=3)
    sobelX = np.uint8(np.absolute(sobelX))
    sobelY = np.uint8(np.absolute(sobelY))
    pair_img_L = cv2.bitwise_or(sobelX, sobelY)

    sobelX = cv2.Sobel(pair_img_R, cv2.CV_16S, 1, 0, ksize=3)
    sobelY = cv2.Sobel(pair_img_R, cv2.CV_16S, 0, 1, ksize=3)
    sobelX = np.uint8(np.absolute(sobelX))
    sobelY = np.uint8(np.absolute(sobelY))
    pair_img_R = cv2.bitwise_or(sobelX, sobelY)
...
def stereo_depth_map(rectified_pair):
    ...
    sbm = cv2.StereoBM_create(numDisparities=32, blockSize=5) #27
    #max_disparity = 32
    #sbm = cv2.StereoSGBM_create(16, max_disparity, 5)
