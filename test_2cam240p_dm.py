...
class USB_Camera:
    ...
    def read(self):
        with self.read_lock:
            ...
            frame = cv2.resize(frame, (320,240)) #3D
            #frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) #2D
...
def load_map_settings(fName):
    ...
    print('[+] Load : SWS={} PFS={} PFC={} MDS={} NOD={} TTH={} UR={} SR={} SPWS={}'.format(SWS,PFS,PFC,MDS,NOD,TTH,UR,SR,SPWS))
...
def start_cameras(calibration):
    ...
    while cv2.getWindowProperty("USB Cameras", 0) >= 0 :
        ...
        _ , left_image=left_camera.read() #3D
        _ , right_image=right_camera.read() #3D
        
        left_image= cv2.flip(left_image,-1) # cv2.rotate(right_image, cv2.ROTATE_180)

        left_image_gray = cv2.cvtColor(left_image, cv2.COLOR_BGR2GRAY) #2D
        right_image_gray = cv2.cvtColor(right_image, cv2.COLOR_BGR2GRAY) #2D

        rectified_pair = calibration.rectify((left_image_gray, right_image_gray))

        disparity = stereo_depth_map(rectified_pair) #3D
        
        camera_images = np.hstack((left_image, right_image, disparity))

        #camera_images = np.hstack((left_image_gray, right_image_gray))
    ...
    print('[*] disp_max = {}, disp_min = {}'.format(disp_max, disp_min))

    print(disparity.dtype)
    print(left_image_gray.dtype)
    print(left_image.dtype)

    print(disparity.shape)
    print(left_image_gray.shape)
...
if __name__ == "__main__":
    ...
    #disparity = np.zeros((cam_width, cam_height), np.uint8)
    sbm = cv2.StereoBM_create(numDisparities=32, blockSize=5)
